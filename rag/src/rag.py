    ...
    def answer(self, question, options=None, k=32, rrf_k=100, save_dir = None, snippets=None, snippets_ids=None):
        ...
        if self.rag:
            if snippets is not None:
                # When snippets are provided manually
                retrieved_snippets = snippets[:k]
                scores = []
            elif snippets_ids is not None:
                # When olny snippet ids are provided manually
            ...
            if len(contexts) == 0:
                contexts = [""]
            # Concatenation: Multiple context documents are combined into one string.
            # Tokenization: The string is tokenized to calculate its token representation.
            # Truncation: The input length is limited to the maximum token length supported by the OpenAI model.
            # Re-formatting: The truncated token sequence is converted back to text
        ...
        if not self.rag:
            ...
            for context in contexts:
                prompt_medrag = self.templates["medrag_prompt"].render(context=context, question=question)
                messages=[
